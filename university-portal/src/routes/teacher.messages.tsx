  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">Communicate with students and colleagues.</p>
      </div>
      ...
      {tab === "inbox" && (
        <div className="space-y-2">
          {inbox.length === 0 ? <p className="py-8 text-center text-sm text-slate-500">No messages in inbox.</p>
          : inbox.map(m => (
            <div key={m.id} onClick={() => markRead(m.id)}
              ...
              <div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-emerald-500/15 text-sm font-bold text-emerald-400">
      ...
      {tab === "sent" && (
        <div className="space-y-2">
          {sent.length === 0 ? <p className="py-8 text-center text-sm text-slate-500">No sent messages.</p>
          : sent.map(m => (
